...
class SNN(torch.nn.Module):
    def __init__(self):
        ...
        self.reward_window_size = 100  # Window size for moving average
    ...
    def calculate_adaptive_reward(self, reward_signal):
        """
        Scale the reward dynamically based on recent performance.

        Args:
            reward_signal (float or Tensor): Current reward signal.

        Returns:
            float or Tensor: Scaled reward signal.
        """
        if isinstance(reward_signal, torch.Tensor):
            reward_signal = reward_signal.item()

        # Update reward history
        self.reward_history.append(reward_signal)
        if len(self.reward_history) > self.reward_window_size:
            self.reward_history.pop(0)

        # Calculate the moving average
        moving_avg = sum(self.reward_history) / len(self.reward_history)

        # Scale reward based on deviation from the moving average
        scaled_reward = (reward_signal - moving_avg) / (1 + abs(moving_avg))  # Prevent division by zero

        return torch.tensor(scaled_reward, dtype=torch.float32)
    ...
    def forward(self, spike, reward_signal=None):
        ...
        for i, block in enumerate(self.blocks):
            ...
            if reward_signal is not None:
                scaled_reward = self.calculate_adaptive_reward(reward_signal)  # Use adaptive reward scaling
                self.update_rstdp(block, i, pre_spike, post_spike, scaled_reward)
    ...
    def update_rstdp(self, block, layer_index, pre_spike, post_spike, reward_signal):
        ...
        if hasattr(block, 'synapse'):
            ...
            delta_e = torch.bmm(
                post_spike.sum(dim=-1).unsqueeze(2),
                pre_spike.sum(dim=-1).unsqueeze(1)
            ).mean(dim=0)
            ...
            self.eligibility_traces[layer_index] += delta_e.view(self.eligibility_traces[layer_index].shape)
            ...
            delta_w = delta_w.squeeze()  # Remove singleton dimensions
