...
DATASET="imdb"# One of: imdb, emotion  #To be added: topic
...
class SNN(torch.nn.Module):
    def __init__(self):
        ...
        self.reward_decay = 0.99  # Decay for the eligibility trace
        self.eligibility_traces = []  # To store eligibility traces
        self.init_eligibility_traces()

    def init_eligibility_traces(self):
        """Initialize eligibility traces for each layer."""
        self.eligibility_traces = []
        for block in self.blocks:
            if hasattr(block, 'synapse'):
                self.eligibility_traces.append(torch.zeros_like(block.synapse.weight))

    def forward(self, spike, reward_signal=None):
        """
        Forward pass with optional reward-modulated STDP.

        Args:
            spike (torch.Tensor): Input spikes to the network.
            reward_signal (torch.Tensor, optional): Reward signal for R-STDP. Default is None.

        Returns:
            torch.Tensor: Output spikes from the network.
            torch.Tensor: Spike counts (if PROFILING is enabled).
        """
        count = [torch.sum((spike[..., 1:]) > 0).to(torch.int64)]  # Initialize spike count

        for i, block in enumerate(self.blocks):
            pre_spike = spike  # Input to the current layer
            spike = block(spike)  # Pass through the current layer
            post_spike = spike  # Output from the current layer

            # Apply reward-modulated STDP if a reward signal is provided
            if reward_signal is not None:
                self.update_rstdp(block, i, pre_spike, post_spike, reward_signal)

            # Record spike count for profiling
            if PROFILING:
                count.append(torch.sum((spike[..., 1:]) > 0).to(torch.int64))

        if PROFILING:
            return spike, torch.LongTensor(count).to(spike.device)
        else:
            return spike, 0

    def update_rstdp(self, block, layer_index, pre_spike, post_spike, reward_signal):
        """
        Update weights based on R-STDP.

        Args:
            block (slayer.block.cuba.Dense): The current dense block of the model.
            layer_index (int): Index of the layer in the network.
            pre_spike (torch.Tensor): Pre-synaptic spike activity (input to the layer).
            post_spike (torch.Tensor): Post-synaptic spike activity (output from the layer).
            reward_signal (torch.Tensor or float or int): Reward signal for modulating synaptic updates.
        """
        if hasattr(block, 'synapse'):
            # Ensure eligibility traces are initialized
            if layer_index >= len(self.eligibility_traces):
                self.eligibility_traces.append(
                    torch.zeros_like(block.synapse.weight.data, device=block.synapse.weight.device)
                )

            # Batch-wise pairwise correlations (outer product for each sample in the batch)
            delta_e = torch.bmm(
                post_spike.sum(dim=-1).unsqueeze(2),  # Shape: [batch_size, num_post_neurons, 1]
                pre_spike.sum(dim=-1).unsqueeze(1)  # Shape: [batch_size, 1, num_pre_neurons]
            )  # Resulting shape: [batch_size, num_post_neurons, num_pre_neurons]

            # Compute the mean over the batch dimension to match the eligibility trace shape
            delta_e_mean = delta_e.mean(dim=0)  # Shape: [num_post_neurons, num_pre_neurons]

            # Ensure eligibility traces match the shape of delta_e_mean
            self.eligibility_traces[layer_index] = self.eligibility_traces[layer_index].view(delta_e_mean.shape)

            # Decay eligibility traces
            self.eligibility_traces[layer_index] *= (1 - self.reward_decay)

            # Update eligibility traces
            self.eligibility_traces[layer_index] += delta_e_mean

            # Ensure reward_signal is a tensor
            if isinstance(reward_signal, (int, float)):
                reward_signal = torch.tensor(reward_signal, dtype=torch.float32, device=block.synapse.weight.device)

            # Apply reward-modulated weight update
            # Reshape delta_w to match the weight shape
            delta_w = self.learning_rate * reward_signal.view(1, 1) * self.eligibility_traces[layer_index]

            # Ensure delta_w matches the shape of the weights (num_post_neurons, num_pre_neurons)
            delta_w = delta_w.squeeze()  # Remove any extra singleton dimensions

            # Update synapse weights
            block.synapse.weight.data += delta_w.view(block.synapse.weight.data.shape)
    ...
    def __init__(self, pairs, embed_loc="embed/binarized/glove-64bit.txt", embed_dim=64, time_per_token=1, spike_freq=1, max_tokens=MAX_TOKENS, spike_rand=False, log_tokens=False):
        ...
        if (True):
            ...
            for item in tqdm(pairs[column], desc=f"WordDataset: Formatting \"{column}\" column..."):
                ...
                if log_tokens:
                    with open(LOG_LOC+"token_log.txt", "a",encoding="utf8") as writefile:
...
def train_model():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for i, (input, target) in enumerate(train_loader):
            ...
            # Forward Pass
            output, count = model(input.to(device),reward_signal)
        ...
        if (VALIDATION_SET):
            for i, (input, target) in enumerate(valid_loader):
                ...
                output, _ = model(input.to(device),reward_signal)
                # Compute Loss
                ...
                if NETWORK_TYPE == "SNN":
                    for sample, ground_truth in zip(output, target):
                        ...
                        counter = 0
                        for label in sample:
                            output_counts.append({"label":counter, "count":torch.sum((label> 0).to(torch.int64)).cpu().item()})
                            counter += 1
